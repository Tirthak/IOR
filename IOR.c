...
static double FindMaxOrMin(double *arr, size_t len, MPI_Op op)
{
  int i;
  double m = arr[0];
                  ...
                  int            access)
{
    ...
    // Rohan: do reduction operation locally for each rank
#if 0
    ...
    for (i = 0; i < 12; i++) {
        op = i % 2 ? MPI_MAX : MPI_MIN;
        reduced[i] = FindMaxOrMin(timer[i], test->repetitions, op);
    }

    // Rohan: for each rank, compute the bandwidths
#if 0
    if (rank == 0) {
#else
    if (rank >= 0) {
...
SummarizeResults(IOR_param_t * test)
{
   ...
   double allMaxWrite  [numTasksWorld][2], allMinWrite [numTasksWorld][ 2], allMaxRead [numTasksWorld][ 2], allMinRead [numTasksWorld][ 2],
          allMeanWrite [numTasksWorld][2], allMeanRead [numTasksWorld][ 2],
          allVarWrite  [numTasksWorld][2], allVarRead  [numTasksWorld][ 2],
          allSdWrite   [numTasksWorld][2], allSdRead   [numTasksWorld][ 2],
          allSumWrite  [numTasksWorld][2], allSumRead  [numTasksWorld][ 2],
          allWriteTimeSum [numTasksWorld], allReadTimeSum [numTasksWorld];
   int i = 0;
   ...
   for (ival=0; ival<2; ival++)
   {
    ...
    // Rohan: Gather results from all the ranks
    MPI_CHECK(MPI_Gather(maxWrite, 2, MPI_DOUBLE, allMaxWrite, 2, MPI_DOUBLE, 0, MPI_COMM_WORLD), "MPI_Gather error");
    MPI_CHECK(MPI_Gather(minWrite, 2, MPI_DOUBLE, allMinWrite, 2, MPI_DOUBLE, 0, MPI_COMM_WORLD), "MPI_Gather error");
    MPI_CHECK(MPI_Gather(meanWrite, 2, MPI_DOUBLE, allMeanWrite, 2, MPI_DOUBLE, 0, MPI_COMM_WORLD), "MPI_Gather error");
    MPI_CHECK(MPI_Gather(sdWrite, 2, MPI_DOUBLE, allSdWrite, 2, MPI_DOUBLE, 0, MPI_COMM_WORLD), "MPI_Gather error");
    MPI_CHECK(MPI_Gather(&writeTimeSum, 1, MPI_DOUBLE, allWriteTimeSum, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD), "MPI_Gather error");
    MPI_CHECK(MPI_Gather(maxRead, 2, MPI_DOUBLE, allMaxRead, 2, MPI_DOUBLE, 0, MPI_COMM_WORLD), "MPI_Gather error");
    MPI_CHECK(MPI_Gather(minRead, 2, MPI_DOUBLE, allMinRead, 2, MPI_DOUBLE, 0, MPI_COMM_WORLD), "MPI_Gather error");
    MPI_CHECK(MPI_Gather(meanRead, 2, MPI_DOUBLE, allMeanRead, 2, MPI_DOUBLE, 0, MPI_COMM_WORLD), "MPI_Gather error");
    MPI_CHECK(MPI_Gather(sdRead, 2, MPI_DOUBLE, allSdRead, 2, MPI_DOUBLE, 0, MPI_COMM_WORLD), "MPI_Gather error");
    MPI_CHECK(MPI_Gather(&readTimeSum, 1, MPI_DOUBLE, allReadTimeSum, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD), "MPI_Gather error");
    ...
    if (rank == 0 && verbose >= VERBOSE_0) 
    {
        ...
        fprintf(stdout, "\n");
        fprintf(stdout, "---------  ---------  ---------  ----------   -------  ---------  ---------  ----------   -------  --------\n");
        // Rohan: print stats for each rank
        for (i = 0; i < numTasksWorld; i++) {
            if (allMaxWrite[0][0] > 0.) {
                fprintf(stdout ,"%s (%d) ", "write", i);
                fprintf(stdout , "%10.2f "    , allMaxWrite[i][0]/MEBIBYTE);
                fprintf(stdout , "%10.2f  "   , allMinWrite[i][0]/MEBIBYTE);
                fprintf(stdout , "%10.2f"     , allMeanWrite[i][0]/MEBIBYTE);
                fprintf(stdout , "%10.2f "    , allSdWrite[i][0]/MEBIBYTE);
                fprintf(stdout , "%10.2f "    , allMaxWrite[i][1]);
                fprintf(stdout , "%10.2f  "   , allMinWrite[i][1]);
                fprintf(stdout , "%10.2f"     , allMeanWrite[i][1]);
                fprintf(stdout , "%10.2f"     , allSdWrite[i][1]);
                fprintf(stdout , "%10.5f   "  , allWriteTimeSum[i]/(double)(test->repetitions));
                fprintf(stdout, "\n");
            }
            if (allMaxRead[0][0] > 0.) {
                fprintf(stdout ,"%s (%d) ", "read", i);
                fprintf(stdout , "%10.2f "    , allMaxRead[i][0]/MEBIBYTE);
                fprintf(stdout , "%10.2f  "   , allMinRead[i][0]/MEBIBYTE);
                fprintf(stdout , "%10.2f"     , allMeanRead[i][0]/MEBIBYTE);
                fprintf(stdout , "%10.2f "    , allSdRead[i][0]/MEBIBYTE);
                fprintf(stdout , "%10.2f "    , allMaxRead[i][1]);
                fprintf(stdout , "%10.2f  "   , allMinRead[i][1]);
                fprintf(stdout , "%10.2f"     , allMeanRead[i][1]);
                fprintf(stdout , "%10.2f"     , allSdRead[i][1]);
                fprintf(stdout , "%10.5f   "  , allReadTimeSum[i]/(double)(test->repetitions));
                fprintf(stdout, "\n");
            }
        }
        // Rohan: disable this for rank 0
#if 0
        if (maxWrite[0] > 0.)
        {
...
TestIoSys(IOR_param_t *test)
{
    ...
    // Rohan: data structure to store per-rank timer info
#if 0
    double         perRankTimers[12][numTasksWorld][test->repetitions];
#endif
    double       * timer[12];
    double         startTime;
    int            i,
    ...
    // Rohan: gather timer info from all the ranks
#if 0
    for (i = 0; i < 12; i++) {
      MPI_CHECK(MPI_Gather(timer[i], test->repetitions, MPI_DOUBLE,
                           perRankTimers[i], test->repetitions, MPI_DOUBLE,
                           0, MPI_COMM_WORLD), "MPI_Gather error");
    }
#endif
